...
export const SteerpathContext = React.createContext<boolean>(false);

interface Props {
  children: ReactNode;
}

export const SteerpathProvider: FC<Props> = ({ children }) => {
  ...
  return (
    <SteerpathContext.Provider value={sdkReady}>
      {children}
    </SteerpathContext.Provider>
  );
};

export default () => useContext(SteerpathContext);
